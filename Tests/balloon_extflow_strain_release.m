...
%
xmin        = -1.0;            % bottom corner of the domain xmin,ymin
ymin        = -1.0;
...
yc          = 0.0;
rad         = 0.25;           % resting radius of the circle
...
tend = 15;            % end time of simulation
...
%
fbg = zeros(Nx,Ny,2);
fbg(:,:,1) = 0.1*((2*pi/Lx)^2+(2*pi/Ly)^2) ...
             *sin(2*pi/Lx*xehoriz).*cos(2*pi/Ly*yehoriz);
fbg(:,:,2) = -0.1*((2*pi/Lx)^2+(2*pi/Ly)^2) ...
             *cos(2*pi/Lx*xevert).*sin(2*pi/Ly*yevert);
...
for k=1:Nt
    ...
    %
    bdy_flux = release_rate*g(st) .*(X(:,2)<0);
...
if recordflag
    close(strainvid)
    !HandBrakeCLI -i strain_release.avi -o strain_release.mp4
    !rm strain_release.avi
end
